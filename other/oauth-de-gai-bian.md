...
Consumer申请Request Token（/oauth/1.0a/request\_token）：
...
在探讨oauth\_timestamp之前，先聊聊oauth\_nonce，它是用来防止重放攻击的，Service Provider应该验证唯一性，不过保存所有的oauth\_nonce并不现实，所以一般只保存一段时间（比如最近一小时）内的数据。
