...
最主要的是 swagger-springmvc 依赖，添加了它之后，我们可以使用 Swagger 特定的注解。Swagger 会查找自己的以及 Spring 特定的注解，这样项目部署后默认可以直接使用 [http://host:port/projectName/api-docs](http://host:port/projectName/api-docs)获取所有WEB层的API接口。返回的是标志JSON格式，所以还要添加 Jckson 依赖。
...
这样其实就可以部署启动，然后直接访问[http://host:port/projectName/api-docs](http://host:port/projectName/api-docs)获取所有通过 `@Controller`、`@RequestMapping`等注解配置的WEB接口。

## 集成 Swagger-ui界面

Swagger 提供了一个静态项目 Swagger-ui，可以帮忙将获得的JSON格式的API优美的展示出来并且帮助测试这些接口。 集成 Swagger-ui 很简单，从[https://github.com/swagger-api/swagger-ui](https://github.com/swagger-api/swagger-ui) 获取其所有的 dist 目录下东西放到需要集成的项目里，比如这里放入 src/main/webapp/WEB-INF/swagger-ui/ 目录下
...
重新部署启动后，访问[http://host:port/projectName/swagger/index.html](http://host:port/projectName/swagger/index.html) 即可看到 Swagger-ui的界面。默认是从连接[http://petstore.swagger.io/v2/swagger.json](http://petstore.swagger.io/v2/swagger.json)获取 API 的 JSON，不过可以在 Swagger-ui 的静态文件的 index.html 中配置为 [http://host:port/projectName/api-docs](http://host:port/projectName/api-docs)。
