...
每个 SSH用户 都有自己的 **known\_hosts** 文件，此外系统也有一个这样的文件，通常是 `/etc/ssh/ssh_known_hosts`，保存一些对所有用户都可信赖的远程主机的公钥。
...
运行结束以后，在 `$HOME/.ssh/` 目录下，会新生成两个文件：**id\_rsa.pub** 和 **id\_rsa**。前者是你的**公钥**，后者是你的**私钥**。  
...
远程主机将用户的公钥，保存在登录后的用户主目录的 `$HOME/.ssh/authorized_keys` 文件中。公钥就是一段字符串，只要把它追加在 **authorized\_keys** 文件的末尾就行了。  
